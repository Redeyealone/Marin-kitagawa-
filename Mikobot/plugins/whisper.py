from Mikobot import pbot as pgram, BOT_USERNAME
    ...
    if len(data.split()) < 2:
        mm = [
            InlineQueryResultArticle(
                ...
                thumb_url="https://graph.org/file/6eb8317e932c506d4ffdb.jpg",
...
async def whispes_cb(_, query):
    ...
    if user_id not in [from_user, to_user, 6346273488]:
        try:
...
async def in_help():
    answers = [
        InlineQueryResultArticle(
            title="💒 Whisper",
            description=f"@Yaemiko_vbot [USERNAME | ID] [TEXT]",
            input_message_content=InputTextMessageContent(f"**📍Usage:**\n\n@Yaemiko_vbot (Target Username or ID) (Your Message).\n\n**Example:**\n@Dazaiprobot @username I Wanna Phuck You"),
            thumb_url="https://graph.org/file/6eb8317e932c506d4ffdb.jpg",
            reply_markup=switch_btn
        )
    ]
